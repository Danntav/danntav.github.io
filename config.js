const SITE_CONFIG = {
  ...
  projects: [
    ...
    ───────────────────────────────────────────────────── */
    {
      ...
      tags: ["Firmware", "Linux", "C", "daemon", "procd", "Embedded", "Makefile", "SDK", ".ipk"],
      ...
      content: [
        ...
        { type: "code", lang: "bash", value: "make -j4 image PROFILE=tl-mr3220-v2 PACKAGES=\"-libiwinfo-lua -liblua -libubus-lua -libuci-lua -lua -luci -luci-app-firewall -luci-base -luci-lib-ip -luci-lib-nixio -luci-mod-admin-full -luci-proto-ipv6 -luci-proto-ppp -luci-theme-bootstrap -uhttpd -uhttpd-mod-ubus kmod-fs-ext4 kmod-usb-storage kmod-scsi-core block-mount kmod-lib-crc32c kmod-crypto-crc32c -ip6tables -kmod-ip6tables -kmod-nf-conntrack6 -kmod-nf-ipt6 -libip6tc -odhcp6c -odhcpd -ppp -ppp-mod-pppoe -kmod-ppp -kmod-pppoe -kmod-pppox -kmod-slhc\"" }, 
        ...
        { type: "text", value: "The SD card was adapted for USB using the generic adapter that came with my 3D printer and mounted to the router's casing using double-sided gel tape. I kept the original USB port and solder another cable on the bottom pins of it.", align: "justify"},
        { type: "image", src: "assets/projects/project_mr3220/sd_adaptation.jpg", caption: "SD card adaptation"},
        ...
        { type: "text", value: "All credits to Vladikoms blog on the following code", bold: true},
        { type: "text", value: "After formating the flash drive with EXT4 fs, connect it to the router and follow the commands:", align: "justify"},
        { type: "code", lang: "bash-router", value: "//Mount the USB drive\nmount /dev/sda1 /mnt/usb\n\n//Check if its mounted\ndf -h\n\n//Copy the /overlay partition to the flash drive\ncp /overlay/* /mnt/usb -a\n\n//Edit fstab file\nvi /etc/config/fstab\n\n//Edit the 'mount' section adding the following\n  config 'mount'\n  option target /overlay\n  option device /dev/sda1\n  optionfstype ext4\n  option options rw,sync\n  option enabled 1\n  option enabled_fsck o\n\n//Enable autostart of fstab when router reboots\n/etc/init.d/fstab enable\n\n//Restart the router\nreboot\n\n//Check result\ndf -h" },
        { type: "text", value: "After extroot", bold: true, size: "large" },
        { type: "text", value: "Check that the hardware booted correctly and loaded the necessary packages. Also check that the /overlay now is being detected. Everything is done via SSH (terminal or other app, like PuTTY, which I like) since the web GUI has been removed.", align: "justify"},
        { type: "code", lang: "bash", value: "opkg list-installed" },
        { type: "image", src: "assets/projects/project_mr3220/bash_overlay.jpeg", caption: "OpenWrt terminal via SSH"},
        { type: "text", value: "Overlay", bold: true, size: "large" },
        { type: "text", value: "Basically, the overlay is the RW (Read-Write) part of the flash. This diagram shows before and after mounting the overlay on the SD card.", align: "justify"},
        { type: "image", src: "assets/projects/project_mr3220/extroot.png", caption: "Before and after extroot"},
               
        { type: "text", value: "Toolchain & First Package", bold: true, size: "large" },
        { type: "text", value: " I downloaded the OpenWrt SDK matching the installed build, cross-compiled a simple hello.c with -Os (optimize for size because the CPU's cache is small), and ran it on the router over SSH. Then packaged it properly as a .ipk using OpenWrt's own package build system (Makefile under package/ and using the standard BuildPackage macros) instead of assembling one by hand. Also added optional postinst/prerm hooks just to see the full install/remove lifecycle.", align: "justify"},
        { type: "code", lang: "makefile", value: "include $(TOPDIR)/rules.mk\n\nPKG_NAME:=hello-router\nPKG_VERSION:=1.0.0\nPKG_RELEASE:=1\n\nSOURCE_DIR:=$(CURDIR)/src\n\ninclude $(INCLUDE_DIR)/package.mk\n\n...\n\ndefine Package/hello-router/install\n\t$(INSTALL_DIR) $(1)/usr/bin\n\t$(INSTALL_BIN) $(PKG_BUILD_DIR)/hello-router $(1)/usr/bin\nendef\n\ndefine Package/hello-router/postinst\ndefine Package/hello-router/prerm\n\n$eval $(call BuildPackage,hello-router))"},
        { type: "text", value: "Note: Just partially showing Makefile above! Check the GitHub repo for the full code.", align: "center"},
      ],
    },
